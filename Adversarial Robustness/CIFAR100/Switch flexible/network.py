...
class CNNBlock(nn.Module):
    def __init__(self, in_chan, nf, n_layers,maxpool,S,a,k):
        ...
        self.rescalings = nn.Parameter(torch.rand(1))
        ...
        for i in range(self.nlayers):
            ...
            torch.nn.init.zeros_(self.convs[i].bias.data)
    ...
    def forward(self, x):
        ...
        x = torch.relu(self.rescalings * self.conv1(x)) #lifting layer
        ...
        for i in range(self.nlayers//2):
          ...
          for k in range(self.S):
              x = x - dtc/self.S * F.conv_transpose2d(self.nl((Ac(x))), Ac.weight, padding=1, stride=1)
          x = x + dte*F.conv_transpose2d(torch.relu((Ac(x))), Ac.weight, padding=1, stride=1)
...
class Network(nn.Module):
    def __init__(self, in_chan, nf1, nf2, nf3, n_l1, n_l2, n_l3,S=1,a=0.5,M=1):
        ...
        self.proj = nn.Linear(self.last,100)
