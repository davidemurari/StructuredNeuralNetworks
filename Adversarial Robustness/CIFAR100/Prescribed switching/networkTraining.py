...
def train(model, margin, criterion, optimizer, scheduler, trainloader, testloader, device='cpu',epochs=100,reg=True,a=0.1,constrain = True, run=None, gamma=0.1):
    ...
    for epoch in range(epochs):
        ...
        for i, inp in enumerate(trainloader):
            ...
            if constrain:
                with torch.no_grad():
                    ...
                    #model.last[-1].weight.data /= max(1,torch.norm(model.last[-1].weight.data,2)/1)
        ...
        with torch.no_grad():
            ...
            print(f"Norm last linear: {torch.norm(model.last[-1].weight.data,2).item()}")
            count = 0
