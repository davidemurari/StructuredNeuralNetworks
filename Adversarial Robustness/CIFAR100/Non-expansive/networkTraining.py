...
def train(model, margin, criterion, optimizer, scheduler, trainloader, testloader, device='cpu',epochs=100,reg=True,a=0.1,constrain = True, gamma = 0.01, run=None):
    
    print("Initial regularization : ",model.reg(),"\n\n")
    print("Regularization coefficient : ",gamma,"\n\n")
    for epoch in range(epochs):
        ...
        for i, inp in enumerate(trainloader):
            ...
            if constrain:
                with torch.no_grad():
                    ...
                    #model.proj.weight.data /= max(1,torch.norm(model.proj.weight.data,2)/1)
                    ...
                    for k in range(3):
                        ...
                        '''for s in np.arange(model.seq[k].nlayers):
                            n, _ = compute_spectral_norm(model.seq[k].convs[s],model.seq[k].storedEigVects[s:s+1],im_size=(model.seq[k].M,model.seq[k].kk,model.seq[k].kk),k=5,device = model.seq[k].convs[s].weight.device)
                            model.seq[k].convs[s].weight.data /= max(1,n)'''
                        
                        model.seq[k].u.data = torch.clamp(model.seq[k].u.data, min=0., max=1.9) #to have a dissipative behaviour
        ...
        with torch.no_grad():
            ...
            print(f"Norm last linear: {torch.norm(model.proj.weight.data,2).item()}")
            count = 0
